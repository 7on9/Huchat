...
var room = {
  ...
  getListMemberOfRoom: (roomCode, callback) => {
    return db.query(
      'CALL PROC_GET_LIST_MEMBER_OF_ROOM(?)',
      [roomCode],
      callback
    );
  },
  getPublicInfoOfRoom: (roomCode, callback) => {
    return db.query(
      'CALL PROC_GET_PUBLIC_INFO_OF_ROOM(?)',
      [roomCode],
      callback
    );
  },
  getHistoryOfChatRoom: (roomCode, callback) => {
    return db.query(
      'CALL PROC_GET_HISTORY_OF_CHAT_ROOM(?)',
      [roomCode],
      callback
    );
  },
  ...
  createDualRoom: async (userName, userName2, roomCode, callback) => {
    return db.query(
      'CALL PROC_CREATE_DUAL_ROOM(?, ?, ?)',
      [userName, userName2, roomCode],
      callback
    );
  },
