...
CREATE PROCEDURE GET_CHAT_ROOM_HISTORY(
    IN N_ROOM_CODE VARCHAR(41))
BEGIN
    SELECT * FROM CHAT_ROOMS_HISTORY WHERE ROOM_CODE = N_ROOM_CODE ORDER BY TIME DESC; 
END; $$
DELIMITER ;
---------------------------------CHAT----------------------------------------
...
CALL PROC_UPDATE_INFO_USER("tttt", "Nguyễn Thành Long", 1111111, true, "a@gmail.com", "00000111");
CALL PROC_CREATE_ROOM("TTTT", "-","LoL", FALSE, TRUE);
CALL PROC_JOIN_ROOM("1544663293TTTT", "Bamboo", "-");
SELECT * FROM ROOMS WHERE ROOM_CODE LIKE "1544663293TTTT" AND ROOMS.PASSWORD = "-";
